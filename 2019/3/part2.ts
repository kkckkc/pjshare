  ...
  for (const [c1, c2] of cartesianProduct(wire1, wire2)) {
    ...
    if (p && !Points.isOrigin(p)) {
      minDistance = Math.min(minDistance, 
        wire1.filter(w => w.idx < c1.idx).reduce((a, c) => a + Lines.length(c), 0) + 
        Lines.length(Lines.make(c1.start, p)) + 
        wire2.filter(w => w.idx < c2.idx).reduce((a, c) => a + Lines.length(c), 0) + 
        Lines.length(Lines.make(c2.start, p)));
    }
  }
